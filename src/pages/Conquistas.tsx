  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#F0EDE8]">Conquistas</h1>
          <p className="text-[#5A5650] mt-1">Marcos e vitórias que alcançamos juntos.</p>
        </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <form onSubmit={handleAdd} className="glass-panel p-6 rounded-2xl shadow-lg border border-white/60">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold text-[#F0EDE8]">Adicionar Conquista</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-[#9A9590] dark:text-[#5A5650] mb-1 uppercase tracking-wider">Título</label>
                  <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="glass-input" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-[#9A9590] dark:text-[#5A5650] mb-1 uppercase tracking-wider">Data</label>
                  <input type="date" required value={date} onChange={e => setDate(e.target.value)} className="glass-input" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-[#9A9590] dark:text-[#5A5650] mb-1 uppercase tracking-wider">Ícone</label>
                  <IconSelect value={icon} onChange={setIcon} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-[#9A9590] dark:text-[#5A5650] mb-1 uppercase tracking-wider">Descrição</label>
            ...
            {filtered.map((a, i) => {
              ...
              return (
                <motion.div key={a.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ delay: i * 0.05 }} layout>
                  <Card className="flex items-start gap-5 hover:bg-[#141418]0 dark:hover:bg-gray-800/80 transition-colors group relative">
                    <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(a)} className="text-[#5A5650] hover:text-[#9A9590] cursor-pointer"><Edit3 size={16} /></button>
                      <button onClick={() => setDeleteId(a.id)} className="text-[#5A5650] hover:text-red-400 cursor-pointer"><Trash2 size={16} /></button>
                    </div>
                    <div className="w-14 h-14 rounded-2xl bg-[#1C1C22] flex items-center justify-center text-[#9A9590] shadow-lg shrink-0"><Icon size={28} /></div>
                    <div className="pr-6 flex-1">
                      <span className="text-xs font-semibold text-[#5A5650] uppercase tracking-wider">{format(parseISO(a.date), "MMMM 'de' yyyy", { locale: ptBR })}</span>
                      <h3 className="text-xl font-bold text-[#F0EDE8] mt-1">{a.title}</h3>
                      <p className="text-[#9A9590] mt-2 leading-relaxed">{a.description}</p>
                      {a.mediaUrl && (
                        <div className="mt-4 media-container bg-[#1C1C22] border border-white/6">
                          {a.mediaType === 'video' ? <video src={a.mediaUrl} controls className="w-full h-auto object-contain" /> :
                           a.mediaType === 'document' ? <a href={a.mediaUrl} download={a.mediaName} className="flex items-center gap-3 p-3 hover:bg-[#141418] text-[#9A9590]"><FileText size={24} className="text-[#5A5650]" /><span className="text-sm font-medium truncate">{a.mediaName || 'Arquivo'}</span></a> :
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0A0A0C]/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="glass-panel shadow-2xl rounded-3xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] border border-white/6">
              <div className="flex justify-between items-center p-6 border-b border-white/6">
                <h2 className="text-xl font-bold text-[#F0EDE8]">Editar Conquista</h2>
