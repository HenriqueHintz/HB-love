...
export const MediaUploadField: React.FC<MediaUploadFieldProps> = ({
  ...
  return (
    <div>
      {label && (
        <label className="block text-xs font-medium text-app-text-secondary dark:text-app-text-muted mb-1 uppercase tracking-wider">
          {label}
        </label>
      )}
      {mediaUrl ? (
        <div
          className={`relative rounded-xl overflow-hidden border border-app-border-light ${height} group bg-app-surface flex items-center justify-center`}
        >
          {mediaType === 'video' ? (
            <video src={mediaUrl} controls className="w-full h-full object-contain" />
          ) : mediaType === 'document' ? (
            <div className="flex flex-col items-center gap-2 text-app-text-secondary dark:text-app-text-muted p-4">
              <FileText size={48} />
        ...
        <label
          className={`flex flex-col items-center justify-center w-full ${height} border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-app-surface/50/50 hover:bg-app-surface/80 transition-colors`}
        >
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            <div className="flex gap-2 text-app-text-muted mb-2">
              <ImageIcon size={24} />
              <Video size={24} />
              <FileText size={24} />
            </div>
            ...
            <p className="text-xs text-app-text-muted mt-1">Máx. 4MB</p>
          </div>
